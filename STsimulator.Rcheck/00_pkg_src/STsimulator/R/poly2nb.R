# revise poly2nb to avoid using spdep package
...
poly2nb <- function(pl, row.names=NULL, snap=sqrt(.Machine$double.eps),
  ...
  if (is.null(row.names)) regid <- row.names(pl) else regid <- NULL
  if (is.null(regid)) {
  ...
  if (queen) attr(ans, "type") <- "queen" else attr(ans, "type") <- "rook"
  #ans <- sym.attr.nb(ans)
