...
ParaDigest=function(input) {
  ...
  num_simulated_datasets <- as.numeric(para$num_simulated_datasets)
  # clean seeds
  simulation_seed_for_each_dataset <- para$simulation_seed_for_each_dataset
  if (num_simulated_datasets>1) {
    para$all_seeds=as.numeric(unlist(strsplit(simulation_seed_for_each_dataset, ",")))
  } else{para$all_seeds=as.numeric(simulation_seed_for_each_dataset)}
  ...
  para$path=ifelse(
    ncol(feature)==1, 1,
    ifelse(para$simulate_spatial_data=="TRUE", 2, 3)
  )
...
ExprLoad=function(para){
  expression_data_file <- as.character(para$expression_data_file)
  expression_data_file_type <- as.character(para$expression_data_file_type)
...
CellFeatureLoad=function(para){
  cell_feature_data_file <- as.character(para$cell_feature_data_file)
  print(cell_feature_data_file)
  ...
  if(type=="Rdata" | type=="RData" ) {
    ...
    feature=as.data.frame(loadRData(cell_feature_data_file))
  }
  if (type=="tsv") {
    #feature=as.data.frame(fread(paste0(path_to_input_dir, cell_feature_data_file)))
    feature=as.data.frame(fread(cell_feature_data_file))
  }
  return(feature)
}
...
ParameterCopula=function(para, expr, feature, ncores=1){
  ...
  if (gene_cor=="TRUE" & copula_input=="NULL") {
   ...
   # save
   out_path_name=paste0(para$path_to_output_dir, "_Copula.RData")
...
ParaCellsNoST=function(para, all_seeds){
  ...
  num_regions <- as.numeric(para$num_regions)
  ...
  # determine cell cell location interactions in each region
   ...
   if (para$custom_cell_location_interactions!="FALSE") {
     tmp1=unlist(strsplit(para$custom_cell_location_interactions, split=";"))
   ...
   num_simulated_datasets <- as.numeric(num_simulated_datasets)
   num_simulated_cells <- as.numeric(num_simulated_cells)
   cell_overlap_cutoff <- as.numeric(cell_overlap_cutoff)
   cell_even_distribution <- as.numeric(cell_even_distribution)
...
ParaCellsST=function(para, feature, all_seeds) {
  ...
  cell_loc=foreach (i = 1:num_simulated_datasets) %dopar% {
    cell.region.loc.model.fc(n=num_simulated_cells,
                                    ...
                                    PointAnno=feature[,1],
                                    PointRegion=R,
                                    window_method=window_method, #TODO: add window method
                                    seed=all_seeds[[i]])
  }
  return(cell_loc)
}
